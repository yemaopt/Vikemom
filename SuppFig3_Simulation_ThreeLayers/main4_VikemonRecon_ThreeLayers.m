...
for ydd=-pinholeRadius:1:pinholeRadius
    for xdd=-pinholeRadius:1:pinholeRadius
        ...
        sample(:,:,ii)=fftshift(ifft2(ifftshift(otfeff1.*fobject1)))+fftshift(ifft2(ifftshift(otfeff2.*fobject2)))+fftshift(ifft2(ifftshift(otfeff3.*fobject3)));
        xp(ii)=xdd;
        yp(ii)=ydd;
    end
end
...
num=0;
cutoff_kg=4*pi/wavelength*na*pixelSize;
    ...
    for theta=0:(5/180*pi):pi
        ...
        for yscan=-(pixelNum-1)/2:(pixelNum-1)/2
            for xscan=-(pixelNum-1)/2:(pixelNum-1)/2
                ...
                pattern=1+cos(kx.*(xp+xscan)+ky.*(yp+yscan));
