...
detectorTotalSize=round(2*0.61*wavelength/na/pixelSize);
detectorElementSize=round(0.2*0.61*wavelength/na/pixelSize);

ii=0;
for ydd=-detectorTotalSize:detectorElementSize:detectorTotalSize
    for xdd=-detectorTotalSize:detectorElementSize:detectorTotalSize
        ii=ii+1;
    ...
    for theta=0:(5/180*pi):pi
        ...
        for yscan=-(pixelNum-1)/2:(pixelNum-1)/2
            for xscan=-(pixelNum-1)/2:(pixelNum-1)/2
                ...
                im(yy,xx,jj)=pattern*squeeze(double(sample(yy,xx,:)));                
            end
        end
    end
end

save modulatedImgSet.mat im
